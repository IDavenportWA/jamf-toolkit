...
**Notes:** `sysadminctl` writes its status to **stderr**, so the `2>&1` redirect is required — without it every user tests as not-enabled and the attribute reports "No users have a secure token" across the whole fleet. The temp stylesheet is created with `mktemp` and removed by an exit trap.
