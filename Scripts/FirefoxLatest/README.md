...
The new copy is staged and verified before the existing install is touched, and the previous version is restored if the swap fails. The quit and relaunch run via `launchctl asuser` so session restore works and Firefox does not end up running as root.
