...
The user's home is resolved via `dscl`, and the generated files are chowned to the user — as root-owned files Teams cannot manage them. Using `~` here would resolve to `/var/root` under a Jamf policy and silently do nothing.
